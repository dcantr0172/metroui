...
local function searchRegex(search)
	local s = "";
	for i = 1, search:utf8len() do
		s = s .. search:utf8sub(i, i) .. ".*";	
	end
	return s;
end
...
local function search_IsInText(search, ...)
	--local searchEx = searchRegex(search);
	local textPy;
	for i = 1, select('#', ...) do
		local text = select(i, ...)
		...
		--[[
		if text then
			textPy = PY:toPinyin(text, false, " ");
		end
		]]
		--if text and (text == search or text:match(search) or (textPy and textPy:match(search))) then
