...
local specWarnFaceRage			= mod:NewSpecialWarningTarget(99945)
...
local timerFaceRageCD			= mod:NewCDTimer(27, 99945)--Has a 27-30 sec cd but off by default as it's subject to wild variation do to traps.
	...
	elseif args:IsSpellID(99837) then--Filter when the dogs get it?
		if args:IsDestTypePlayer() then
			...
			if not self:IsTank() and not self:IsHealer() then
				sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\killcrystal.mp3")
			end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(100002, 100031) then	--100002 confirmed .. all/right ids?
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\spear.mp3")
		sndWOP:Schedule(4, "Interface\\AddOns\\DBM-Core\\extrasounds\\firecircle.mp3")
		specWarnSpear:Show()
		timerSpearCD:Start()
...
function mod:SPELL_CAST_SUCCESS(args)
	if args:IsSpellID(99945, 99947) then--is 99945 even used?, cause my 10 man logs only had 99947 for Spell cast success.
		...
		if not self:IsTank() and not self:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\"..GetLocale().."\\killrageface.mp3")
		end
