...
function Drag:OnEnter()
	...
	GameTooltip:SetText(format(L['Bar: %s'], self:GetText():gsub('^%l', string.upper)), 1, 1, 1)
    --print(self:GetText())
    --self.owner.id
  --  local bar_name = self.owner.id
    --if(not tonumber(bar_name)) then
      --  bar_name = L[bar_name]
    --end
--	GameTooltip:SetText(format(L['Bar: %s'], bar_name), 1, 1, 1)
	--GameTooltip:SetText(format(L['Bar: %s'], self:GetText():gsub('^%l', string.upper)), 1, 1, 1)
