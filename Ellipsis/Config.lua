...
local function LoadEllipsisOptions()
	if (not IsAddOnLoaded('Ellipsis_Options')) then
		...
		if (loaded) then Ellipsis:LoadOptions() end
	end
end
...
function Ellipsis:InitializeConfig()
	...
	hooksecurefunc('InterfaceOptionsFrame_OnShow', LoadEllipsisOptions) -- so the options are loaded on frame open if they exist
end
...
function Ellipsis:ApplyConfigSettings()
	...
	self:ConfigureAnchors() -- also configures the cooldown base
	...
	self:SetSecondaryScan(GetNumPartyMembers() > 0 or GetNumRaidMembers() > 0, UnitExists('pet'))
