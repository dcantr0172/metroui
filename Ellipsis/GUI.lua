...
do
	...
	function Ellipsis:InitializeGUI()
		...
		f:SetMovable(true);
		f:SetPoint('BOTTOMRIGHT', UIParent, -200,400);
		f.dragTex = f:CreateTexture(nil, 'BORDER');
		f.dragTex:SetAllPoints();
		f.dragTex:SetTexture(0.5, 0.5, 0.5, 0.5);
		...
		f.dragStr:SetText(L.title..L.Targets);
		local lockBtn = CreateFrame("Button", nil, f);
		f.lockBtn = lockBtn;
		lockBtn:SetNormalTexture([[Interface\AddOns\Ellipsis\lockButton]]);
		lockBtn:SetHighlightTexture([[Interface\BUTTONS\ButtonHilight-Round]], "ADD");
		lockBtn:SetSize(32, 32);
		lockBtn:SetPoint("RIGHT", f, "RIGHT", 3, 0);
		lockBtn:SetScript("OnEnter", function()
			GameTooltip:SetOwner(lockBtn, "ANCHOR_RIGHT")
			GameTooltip:SetText("Ellipsis");
			GameTooltip:AddLine(L.mouseCommand, 1, 1, 1);
			GameTooltip:AddLine(L.howToConfig, 1,1,1)  
			GameTooltip:Show();
		end)
