...
mod:CreatePage(true,"");
mod:CreateButton(L["Feats"],ACHIEVEMENTS.." & "..STATISTICS, L["Right Click for extended menu"]);
...
-- Options
ex.options[#ex.options + 1] = { var = "inspectFeats", default = true, label = L["Request Achievement Data"], tip = L["Asks the server for achievement data when inspecting a player"] };
...
function mod:OnButtonClick(button)
	...
	if (button == "LeftButton") then
		if (IsShiftKeyDown()) then
			AzDialog:Show(L["Enter new feats filter..."],cfg.featsFilter,FeatsFilterOkayFunc);
		end
	-- right
...
function mod.MenuInit(parent,list)
	-- filter
	list[1].text = L["Filter"]; list[1].header = 1;
	list[2].text = L["Set Filter..."]; list[2].value = 1;
	list[3].text = L["Hide Complete"]; list[3].value = 2; list[3].checked = cfg.featsHideComplete;
	list[4].text = L["Hide Incomplete"]; list[4].value = 3; list[4].checked = cfg.featsHideIncomplete;
	-- sort
	list[5].header = 1;
	list[6].text = L["Sort Method"]; list[6].header = 1;
	for index, method in ipairs(featsSortMethods) do
		list[index + 6].text = L["Sort by "]..method; list[index + 6].value = method; list[index + 6].checked = (cfg.featsSort == method);
	end
end
	...
	if (IsModifiedClick("CHATLINK")) then
		...
		if (ChatFrameEditBox:IsVisible()) then
			if (IsControlKeyDown() and not isPlayer) or (not IsControlKeyDown() and isPlayer) then
				ChatFrameEditBox:Insert(GetAchievementLink(entry.id));
			else
				ChatFrameEditBox:Insert(ex.info.name..": "..entry.name..(entry.value and ": "..entry.value:gsub("|T[^|]-GoldIcon[^|]-|t","g"):gsub("|T[^|]-SilverIcon[^|]-|t","s"):gsub("|T[^|]-CopperIcon[^|]-|t","c") or ""));
			end
...
--	if (IsAltKeyDown() and not isPlayer) or (not IsAltKeyDown() and isPlayer) then -- wtb xor
	...
	gtt:AddDoubleLine(L["Achievement Points"],tostring(entry.points),0.25,.75,0.25,1,1,1);
	if (entry.year and entry.month and entry.day) then
		gtt:AddDoubleLine(L["Date Completed"],format("%d.%.2d.%.2d",entry.year + 2000,entry.month,entry.day),0.25,0.75,0.25,1,1,1);
	end
	gtt:AddDoubleLine(L["Cat / Achievement ID"],entry.catId.." / "..entry.id,0.25,0.75,0.25,1,1,1);
	-- Criteria
 	local nCriteria = GetAchievementNumCriteria(entry.id);
 	if (nCriteria and nCriteria > 0) then
		gtt:AddLine(" ");
		gtt:AddLine(L["Achievement Criteria |cffffffff"]..nCriteria);
		local index = 1;
		while (true) do
...
local function FeatsDropDown_InitFunc(dropDown,list)
	...
	tbl.text =L[ "Feats of Strength Query"]; tbl.value = -3; tbl.stub = -3;
	-- Achievements
	local tbl = list[#list + 1];
	tbl.text = L["All Achievements |cffc0c0c0("]..GetComparisonCategoryNumAchievements(-1).."/"..GetCategoryNumAchievements(-1)..")"; tbl.value = -1; tbl.stub = -1;
	...
	-- Statistics
	local tbl = list[#list + 1];
	tbl.text = L["All Statistics |cffc0c0c0("]..GetCategoryNumAchievements(-2)..")"; tbl.value = -2; tbl.stub = -2;
...
cat:SetText(L["Select Category:"]);

-- DropDown
catDropDown = AzDropDown.CreateDropDown(mod.page,200,true,FeatsDropDown_InitFunc,function(dropDown,entry) cfg.featsCat = entry.value; cfg.featsStub = entry.stub; mod:QueryFeats(); end);
