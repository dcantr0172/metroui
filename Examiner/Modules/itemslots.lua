...
function mod:UpdateItemSlots()
	for index, button in ipairs(self.slotBtns) do
		...
		elseif (GetItemInfo(link)) then
			local _, _, itemRarity, _, _, _, _, _, _, itemTexture = GetItemInfo(link);
...
local function OnEnter(self,motion)
	...
	elseif (self.realLink) then
		...
		gtt:AddLine(L["This item was not in the local item cache, click this button to reload the cached player, so the item will update."],1,1,1,1);
		gtt:Show();
	else
		gtt:SetText(_G[self.slotName:upper()]);
	end
...
local function OnClick(self,button)
	if (self.link) then
		if (button == "RightButton") then
			AzMsg(L["---|2 Gem Overview for "]..select(2,GetItemInfo(self.link)).." |r---");
			for i = 1, 3 do
		...
		elseif (button == "LeftButton") then
			...
			elseif (IsModifiedClick("CHATLINK")) and ( editBox ) then
