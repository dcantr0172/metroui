...
-- Options
ex.options = {
	{ var = "makeMovable", default = true, label = L["Make Examiner Movable"], tip = L["To freely move Examiner around, enable this option, otherwise it will behave like a normal frame, such as the Quest Log or Spellbook"] },
	{ var = "autoInspect", default = false, label = L["Auto Inspect on Target Change"], tip = L["With this option turned on, Examiner will automatically inspect your new target when you change it."] },
	{ var = "clearInspectOnHide", default = false, label = L["Clear Inspect Data on Hide"], tip = L["When Examiner is hidden, this option will clear inspection data, this will free up some memory."] },
	{ var = "percentRatings", default = false, label = L["Show Ratings in Percentage"], tip = L["With this option enabled, ratings will be displayed in percent relative to the inspected person's level."] },
	{ var = "combineAdditiveStats", default = true, label = L["Combine Additive Stats"], tip = L["This option will combine certain stats which stacks with others. This include Spell Damage to specific spell schools and AP to Ranged AP."] },
};
...
local function Examiner_OnShow(self)
	...
	getglobal("ExaminerSideBar"):ClearAllPoints()
	getglobal("ExaminerSideBar"):SetParent(ex)
	getglobal("ExaminerSideBar"):SetPoint("TOPLEFT", ex, "TOPRIGHT", -35, -12);
	getglobal("ExaminerSideBar"):Show()

	if IcetipDB and IcetipDB["mousetarget"].showTalent == true then
		fixIce = true;
		IcetipDB["mousetarget"].showTalent = false
	end
end
...
local function Examiner_OnHide(self)
	...
	getglobal("ExaminerSideBar"):SetParent(PaperDollFrame)
	getglobal("ExaminerSideBar"):ClearAllPoints()
	getglobal("ExaminerSideBar"):SetPoint('TOPLEFT', PaperDollFrame, 'TOPRIGHT', -35, -14)

	if IcetipDB and fixIce then
		fixIce = false;
		IcetipDB["mousetarget"].showTalent = true;
	end
end
...
function ex:VARIABLES_LOADED(event)
	...
	cache = Examiner_Cache;
        ...
        --paperdoll frame ex
        if (cfg.statframeCollapsed == nil) then
            cfg.statframeCollapsed = true;
        end
...
function ex:UnitGuildString()
	...
	if (info.raceFixed) then
		if (info.guild and info.guildRank and info.guildIndex) then
			...
			return info.guildRank.." <"..info.guild..">";
		end
...
--update 0515 wolftankk@gmail.com
...
ex.slashFuncs = {
	...
	-- Clear Cache -- Az: there is no refresh of the cache list in case it was visible here!
