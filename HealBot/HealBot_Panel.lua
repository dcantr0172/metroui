...
function HealBot_Panel_ToggelHealTarget(hbGUID)
    if not HealBot_UnitID[hbGUID] or HealBot_UnitID[hbGUID]=="target" then return end
    mti=0
...
function HealBot_Panel_TestBarsOn(showHeaders)
    ...
    if Healbot_Config_Skins.SelfHeals[Healbot_Config_Skins.Current_Skin]==1 then
        ...
        if b then
            ...
            if not HealBot_BottomAnchors then HeaderPos[j+1] = HEALBOT_OPTIONS_SELFHEALS end
            if HealBot_BottomAnchors then HeaderPos[j+2] = HEALBOT_OPTIONS_SELFHEALS end
            noBars=noBars-1
            i=i+1
        end
    end
    if Healbot_Config_Skins.TankHeals[Healbot_Config_Skins.Current_Skin]==1 and HealBot_Globals.noTestTanks>0 and HealBot_Globals.noTestTanks<noBars then
        if not HealBot_BottomAnchors then HeaderPos[i+1] = HEALBOT_OPTIONS_TANKHEALS end
        for j=1+i,HealBot_Globals.noTestTanks+i do
            b=HealBot_Action_SetTestButton(j)
            if b then
                HealBot_Panel_TestBarShow(j,b)
                noBars=noBars-1
    ...
    if noBars>0 then
        ...
        if not HealBot_BottomAnchors then HeaderPos[i+1] = HEALBOT_OPTIONS_GROUPHEALS end
        for j=1+i,z+i do
            b=HealBot_Action_SetTestButton(j)
            if b then
                HealBot_Panel_TestBarShow(j,b)
        ...
        if HealBot_BottomAnchors then HeaderPos[i+1] = HEALBOT_OPTIONS_GROUPHEALS end
    end
    if HealBot_Globals.noTestTargets>0 and HealBot_Globals.noTestTargets<noBars then
        if not HealBot_BottomAnchors then HeaderPos[i+1] = HEALBOT_OPTIONS_MYTARGET end
        for j=1+i,HealBot_Globals.noTestTargets+i  do
            b=HealBot_Action_SetTestButton(j)
            if b then
                HealBot_Panel_TestBarShow(j,b)
                noBars=noBars-1
    ...
    HealBot_noTestPets=HealBot_Globals.noTestPets
    if HealBot_Globals.noTestPets<=noBars then
        noBars=noBars-HealBot_Globals.noTestPets
    else
    ...
    if HealBot_noTestPets>0 then
        ...
        for j=1,HealBot_noTestPets do
            if Healbot_Config_Skins.GroupPetsBy5[Healbot_Config_Skins.Current_Skin] == 1 and j%5 == 0 then HeaderPos[i+2] = HEALBOT_OPTIONS_PETHEALS end
            b=HealBot_Action_SetTestButton(i+1)
            if b then
                HealBot_Panel_TestBarShow(i+1,b)
                i=i+1
            end            
        end
...
function HealBot_Panel_TestBarShow(index,button)
    ...
    if not HealBot_setTestCols then
        if (Healbot_Config_Skins.HlthBarColour[Healbot_Config_Skins.Current_Skin] >= 2) then
            if (Healbot_Config_Skins.HlthBarColour[Healbot_Config_Skins.Current_Skin] == 3) then
                if index==1 then
...
function HealBot_Panel_PanelChanged(showHeaders, disableHealBot)
    ...
    if x then -- Crash Protection
        ...
        if Healbot_Config_Skins.SelfHeals[Healbot_Config_Skins.Current_Skin]==1 or disableHealBot then
        ...
        if not disableHealBot then
            ...
            if Healbot_Config_Skins.SetFocusBar[Healbot_Config_Skins.Current_Skin]==1 then
                if UnitExists("focus") or Healbot_Config_Skins.FocusBarAlwaysShow[Healbot_Config_Skins.Current_Skin]==1 then
                    MyHealTargets=true
                end
            end
            ...
            if Healbot_Config_Skins.PetHeals[Healbot_Config_Skins.Current_Skin]==1 and i<54 then
                ...
                if nraid>0 then
                    for j=1,40 do
                        ...
                        if UnitExists(xUnit) and not HealBot_TrackNames[xGUID] and not HealBot_CheckedTargets[xGUID] and HealBot_TrackNames[UnitGUID(pUnit)] and not UnitUsingVehicle(pUnit) then
                            if not HealBot_TrackNotVisible[xGUID] and i<54 then
                                i = i+1;
                                HealBot_UnitName[xGUID] = uName;
                                HealBot_UnitNameGUID[uName]=xGUID
                                HealBot_GroupGUID[xGUID]=HealBot_GroupGUID[UnitGUID(pUnit)]
                        ...
                        if Healbot_Config_Skins.GroupPetsBy5[Healbot_Config_Skins.Current_Skin] == 1 then
                            if not HealBot_BottomAnchors and i-k == 6 then HeaderPos[i-5] = HEALBOT_OPTIONS_PETHEALS.." 1" end
                            if i>k and ((i-k)%5 == 0) then                           
                                HealBot_Panel_SubSort(hbincSort)        
                                numHeaders=numHeaders+1 
                                HeaderPos[i+1] = HEALBOT_OPTIONS_PETHEALS.." "..((i-k)/5)+(HealBot_BottomAnchors and 0 or 1)
                            end
                        end
                    end
                    if Healbot_Config_Skins.GroupPetsBy5[Healbot_Config_Skins.Current_Skin] == 1 and HealBot_BottomAnchors and i-k == 5 then HeaderPos[i+1] = HEALBOT_OPTIONS_PETHEALS end
                else
                    for j=1,4 do
                        xUnit="partypet"..j;
                        xGUID=HealBot_UnitGUID(xUnit)
            ...
            if Healbot_Config_Skins.VehicleHeals[Healbot_Config_Skins.Current_Skin]==1 and i<55 then
                for x,_ in pairs(HealBot_CheckedTargets) do
                    HealBot_CheckedTargets[x]=nil;
                end
                ...
                if nraid>0 and i<55 then
                    for j=1,40 do
                        ...
                        if xGUID and not HealBot_TrackNames[xGUID] and not HealBot_CheckedTargets[xGUID] and HealBot_TrackNames[UnitGUID(pUnit)] and UnitUsingVehicle(pUnit) and uName then
                            if not HealBot_TrackNotVisible[xGUID] and i<55 then
                                i = i+1;
                                HealBot_UnitName[xGUID] = uName;
                                HealBot_UnitNameGUID[uName]=xGUID
                                HealBot_GroupGUID[xGUID]=HealBot_GroupGUID[UnitGUID(pUnit)]
            ...
            if Healbot_Config_Skins.TargetHeals[Healbot_Config_Skins.Current_Skin]==1 then
                ...
                if UnitExists(xUnit) and UnitIsFriend("player",xUnit) and UnitHealth(xUnit)>99 and (Healbot_Config_Skins.TargetBarAlwaysShow[Healbot_Config_Skins.Current_Skin]==1 or k==1) then
                    uName=UnitName(xUnit)
...
function HealBot_Panel_SetupBars(showHeaders)
    ...
    if showHeaders then 
        ...
        table.foreach(HealBot_Action_HealButtons, function (x,xButton)
            ...
            elseif HealBot_UnitName[xButton.guid] then
    ...
    elseif Healbot_Config_Skins.GroupsPerCol[Healbot_Config_Skins.Current_Skin]==1 then
        ...
        table.foreach(HealBot_Action_HealButtons, function (x,xButton)
            if UnitName(xButton.unit) then
                uName=UnitName(xButton.unit);
            elseif HealBot_UnitName[xButton.guid] then -- Combat Protection 
                uName=HealBot_UnitName[xButton.guid];
                xButton:Show()
            elseif HealBot_cpOn and HealBot_cpName[xButton.unit] then -- Crash Protection 
                uName=HealBot_cpName[xButton.unit];
    ...
    if HealBot_Globals.HideOptions==1 then
    ...
    if hbPanelShowhbFocus and not HealBot_IsFighting and UnitName("target") and UnitName("target") and HealBot_Globals.FocusMonitor[UnitName("target")] then
