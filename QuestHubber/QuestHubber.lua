...
local DATA_FAIL = L["Failed retrieving quest data from server %d time(s)."]
local DATA_SUCCEED = L["Sever returned %d completed quests after %d failed query attempt(s)."]
local FACTION_MAP = {
	Alliance = 1,
	Horde = 2,
};
...
core.numPlayerQuests = 0;		-- number of quests completed above
...
local dataObj = ldb:NewDataObject("QuestHubber", {
	...
	OnTooltipShow = function(self)
		...
		self:ClearLines()
		self:AddLine(("QuestHubber - %s"):format(GetRealZoneText()))
		...
		self:AddLine(" ")
		self:AddLine("|cFFEDA55F点击|r 打开世界地图", 0.2, 1, 0.2, 1)
		--self:AddLine("|cFFEDA55FRight-Click|r to open the QuestHubber options", 0.2, 1, 0.2, 1)
		--self:AddLine("|cFFEDA55FCtrl-Click|r to toggle low level quest tracking", 0.2, 1, 0.2, 1)
		self:AddLine("|cFFEDA55FCtrl-点击|r 显示/隐藏未完成的低级任务", 0.2, 1, 0.2, 1)
	end,
})
...
--	-- self:Print(format("Failed retrieving quests %d time(s). Current status: %s.", self.failedQueries, self.serverQueried and "Successful" or "Waiting"))
...
function core:OnEnable()
	...
	self:RegisterEvent("QUEST_QUERY_COMPLETE"); 		-- get quests completed
...
function core:QueryQuestsCompleted()
	...
	self:ScheduleTimer(function()
		if (not next(self.playerQuests)) then
			self:Debug("Could not retrieve quest data, trying again.");
			self:QueryQuestsCompleted();
			self.failedQueries = self.failedQueries + 1;
			options.args.status.name = DATA_FAIL:format(self.failedQueries)
			-- options.args.status.hidden = false
		else
			self.mapModule.note:Hide();
			for _ in pairs(self.playerQuests) do
				self.numPlayerQuests = self.numPlayerQuests + 1
			end
			options.args.status.name = DATA_SUCCEED:format(self.numPlayerQuests, self.failedQueries)
			-- options.args.status.hidden = true
			self.serverQueried = true;
		end
	end, 10);
end
...
function core:QUEST_QUERY_COMPLETE()
...
function core:SetupMapModule()
	...
	frame:SetPoint("TOPRIGHT", WorldMapDetailFrame, "TOPRIGHT", 0, -2);
	frame:Hide();
	frame:EnableMouse(1);
	...
	frame.text:SetText("可解任务");
	frame.text:SetPoint("LEFT", frame.check, "RIGHT");

	frame.note = frame:CreateFontString();
	frame.note:SetFont(font, 10, "NONE");
	frame.note:SetWidth(250);
	frame.note:SetWordWrap(true);
	-- frame.note:SetIndentedWordWrap(0);
	frame.note:SetJustifyH("LEFT");
	frame.note:SetText(L["Note: Your quest completion data has not been retrieved from the server. This could take up to a minute."]);
	frame.note:SetPoint("TOPLEFT", frame.check, "BOTTOMLEFT");
