...
local L = setmetatable({
    ["|cFF33FF99Steal Your Carbon|r: "] = '|cFF33FF99魔兽精灵自动补购|r: ',
    ["Upgrading %s to %s"] = "从 %s 提升至 %s",
    ["Buying %s x%d"] = "购买 %s x%d",
    ["Spent"] = "花费",
} or GetLocale() == "zhTW" and {
    ["|cFF33FF99Steal Your Carbon|r: "] = '|cFF33FF99魔獸精靈自動補購|r: ',
    ["Upgrading %s to %s"] = "从 %s 提升至 %s",
    ["Buying %s x%d"] = "购买 %s x%d",
    ["Spent"] = "花费",
} or {
...
function StealYourCarbon:Print(...) ChatFrame1:AddMessage(string.join(" ", L["|cFF33FF99Steal Your Carbon|r: "], ...)) end
function StealYourCarbon:PrintF(fmsg, ...) ChatFrame1:AddMessage(string.format(L["|cFF33FF99Steal Your Carbon|r: "]..fmsg, ...)) end
...
function StealYourCarbon:UpgradeWater()
	...
	if found and buy > 0 then
		...
		if self.db.chatter then self:PrintF(L["Upgrading %s to %s"], select(2, GetItemInfo(oldid)), select(2, GetItemInfo(found))) end
	end
end
...
function StealYourCarbon:MERCHANT_SHOW()
	local hastradebag = flase --HasTradeskillBag()
	...
	for i=1,GetMerchantNumItems() do
		...
		if itemID and stocklist[itemID] then
			...
			if needed > 0 then
				...
				if self.db.chatter then self:PrintF(L["Buying %s x%d"], select(2, GetItemInfo(itemID)), tobuy) end
	...
	if spent > 0 and self.db.chatter then self:Print(L["Spent"], GS(spent)) end
end
