...
function TrinketMenu.GetNameByID(id)
	if id==0 then
		return L["-- stop queue here --"],"Interface\\Buttons\\UI-GroupLoot-Pass-Up",1
	else
...
function TrinketMenu.SortTooltip(self)
	...
	if itemLink and TrinketMenuOptions.ShowTooltips=="ON" then
		...
		TrinketMenu.OnTooltip(self,L["Stop Queue Here"],L["Move this to mark the lowest trinket to auto queue.  Sometimes you may want a passive trinket with a click effect to be the end (Burst of Knowledge, Second Wind, etc)."])
	end
end
			...
			if not ready or enable==0 or (TrinketMenuQueue.Stats[list[i]] and TrinketMenuQueue.Stats[list[i]].priority) then
				if TrinketMenu.TrinketNearReady(list[i]) then
					if GetItemCount(list[i])>0 and not IsEquippedItem(list[i]) then
						_,bag,slot = TrinketMenu.FindItem(list[i])
						if bag then
...
function TrinketMenu.SetQueue(which,...)
	...
	if not which or not tonumber(which) or which<0 or which>1 then
		DEFAULT_CHAT_FRAME:AddMessage(errorstub..L["First parameter must be 0 for top trinket or 1 for bottom."])
		return
	end
	if select("#",...)<1 then
		DEFAULT_CHAT_FRAME:AddMessage(errorstub..L["Second parameter is either ON, OFF, PAUSE, RESUME or the beginning of a list of trinkets in a sort order."])
		return
	end
...
--		table.setn(TrinketMenuQueue.Sort[which],0)
		...
		DEFAULT_CHAT_FRAME:AddMessage(errorstub..L[" Expected ON, OFF, PAUSE, RESUME or SORT+list"])
	end
...
function TrinketMenu.GetQueue(which)
	if not which or not tonumber(which) or which<0 or which>1 then
		DEFAULT_CHAT_FRAME:AddMessage(L["|cFFBBBBBBTrinketMenu.GetQueue:|cFFFFFFFF Parameter must be 0 for top trinket or 1 for bottom."])
		return
	end
