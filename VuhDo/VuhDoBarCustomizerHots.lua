...
local tStarted;
local function VUHDO_customizeHotIcons(aButton, aHotName, aRest, aTimes, anIcon, aDuration, aMode, aShieldCharges, aColor, anIndex, aClipL, aClipR, aClipT, aClipB)
...
local function VUHDO_updateHotIcons(aUnit, aHotName, aRest, aTimes, anIcon, aDuration, aMode, aColor, aHotSpellName, aClipL, aClipR, aClipT, aClipB)
	...
	tShieldCharges = aMode ~= 2 -- if not our shield don't show remaining absorption
		and VUHDO_getShieldLeftCount(aUnit, aHotSpellName or aHotName) or 0;
		...
		if (aHotName == tHotName) then
			...
			if (tIsMatch) then
				if (tIndex >= 6 and tIndex <= 8) then
					...
					for _, tButton in pairs(tAllButtons) do
						VUHDO_customizeHotIcons(tButton, aHotName, aRest, aTimes, anIcon, aDuration, aMode, tShieldCharges, aColor, tIndex, aClipL, aClipR, aClipT, aClipB);
					end
				end
			end
		end
...
function VUHDO_removeHots(aUnit)
	...
	for _, tButton in pairs(tAllButtons) do
		for tCnt = 1, 5 do
			...
			VUHDO_getBarIconClockOrStub(tButton, tCnt, true):SetAlpha(0);
		end
	...
	if (VUHDO_shouldScanUnit(aUnit)) then
		...
		for tCnt = 1, 255 do
			if (tDebuffOffset == nil) then
				tBuffName, _, tBuffIcon, tStacks, _, sDuration, tExpiry, tCaster, _, _, tSpellId = UnitBuff(aUnit, tCnt);
				if (tBuffIcon == nil) then
					tDebuffOffset = tCnt - 1;
				end
			end
			if (tDebuffOffset ~= nil) then -- Achtung kein elseif
				tBuffName, _, tBuffIcon, tStacks, _, sDuration, tExpiry, tCaster, _, _, tSpellId = UnitDebuff(aUnit, tCnt - tDebuffOffset);
				if (tBuffIcon == nil) then
					break;
				end
			end
			...
			elseif(tHotFromBuff ~= nil) then -- Hot buff cached
				tRest = tExpiry - tNow;
				if (tRest > 0) then
					VUHDO_snapshotHot(tHotFromBuff, tRest, tStacks, tBuffIcon, tIsMine, sDuration, aUnit, tExpiry);
				end
				...
				for tHotCmpName, _ in pairs(VUHDO_ACTIVE_HOTS) do
					...
					if (tDiffIcon == tBuffIcon
						...
						tRest = tExpiry - tNow;
						if (tRest > 0) then
							VUHDO_snapshotHot(tHotCmpName, tRest, tStacks, tBuffIcon, tIsMine, sDuration, aUnit, tExpiry);
						end
		...
		if (VUHDO_isPanelVisible(tCnt)) then
			...
			for tCnt2 = 1, 51 do -- VUHDO_MAX_BUTTONS_PANEL
				...
				for tCnt3 = 1, 5 do
					...
					VUHDO_getBarIconClockOrStub(tButton, tCnt3, true):SetAlpha(0);
				end
