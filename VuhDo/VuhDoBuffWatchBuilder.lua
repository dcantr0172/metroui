...
local function VUHDO_addBuffSwatch(aBuffPanel, aGroupName, someVariants, aBuffTarget, aCategSpec)
	...
	tMaxTargetName = VUHDO_getBuffVariantMaxTarget(someVariants)[1];
...
--
local tCategBuffs, tBuffVariants;
local function VUHDO_getBuffVariants(aBuffName)
	for _, tCategBuffs in pairs(VUHDO_CLASS_BUFFS[VUHDO_PLAYER_CLASS]) do
		for _, tBuffVariants in pairs(tCategBuffs) do
			if (aBuffName == tBuffVariants[1][1]) then
...
local function VUHDO_addBuffPanel(aCategorySpec)
	...
	tLabelText = VUHDO_BUFF_SETTINGS[tCategName]["buff"] or tMaxVariant[1];
	...
	tIcon = (VUHDO_BUFFS[tLabelText] ~= nil and VUHDO_BUFFS[tLabelText]["icon"] ~= nil)
		...
		or VUHDO_BUFFS[tMaxVariant[1]]["icon"];
...
local function VUHDO_addAllBuffPanels()
	...
	for _, _ in pairs(tAllClassBuffs) do
		for tCategSepc, _ in pairs(tAllClassBuffs) do
			...
			local tNumber = VUHDO_BUFF_ORDER[tCategSepc] == nil
				and tonumber(strsub(tCategSepc, 1, 2))
				or VUHDO_BUFF_ORDER[tCategSepc];
