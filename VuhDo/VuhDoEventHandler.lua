...
local function VUHDO_init()
	...
	if (not VUHDO_CONFIG["doCompress"]) then
		VUHDO_initCompressTables();
		collectgarbage('collect');
		VUHDO_CONFIG["doCompress"] = true;
	end
...
local tUnit;
local tEmptyRaid = { };
function VUHDO_OnEvent(_, anEvent, anArg1, anArg2, anArg3, anArg4, anArg5, anArg6, anArg7, anArg8, anArg9, anArg10, anArg11, anArg12, anArg13, anArg14, anArg15, anArg16)
	...
	if ("COMBAT_LOG_EVENT_UNFILTERED" == anEvent) then
		if (VUHDO_VARIABLES_LOADED) then
			...
			if (sShowShieldAbsorb) then
				VUHDO_parseCombatLogShieldAbsorb(anArg2, anArg4, anArg8, anArg13, anArg16);
			end
		end
...
--
function VUHDO_setPanelsVisible(anIsVisible)
	if (not InCombatLockdown()) then
...
local VUHDO_ALL_EVENTS = {
	...
	"UNIT_SPELLCAST_STOP", "UNIT_SPELLCAST_FAILED", "UNIT_SPELLCAST_SENT", "UNIT_SPELLCAST_SUCCEEDED",
	...
	"READY_CHECK", "READY_CHECK_CONFIRM", "READY_CHECK_FINISHED", "ROLE_CHANGED_INFORM",
	"CVAR_UPDATE",
	"INSPECT_READY",
