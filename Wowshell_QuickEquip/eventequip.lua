...
-- 事件换装
-- $Revision: 2063 $
-- $Date: 2009-04-22 11:22:20 +0800 (Wed, 22 Apr 2009) $
...
local addon = QuickEquip:NewModule(MODULENAME, "AceEvent-3.0", "AceHook-3.0");
...
function addon:OnInitialize()
	--self.db = QuickEquip.db:RegisterNamespace(MODULENAME, {profile = {}});
	--db = self.db.profile
	--self:SetEnabledState(QuickEquip.db.profile.enabledEE);
...
function addon:CreateEEFrame()
	...
	local function eventOnClick(self)
		...
		if uset[eventSelected] then
			local setindex = QuickEquip:GetSetID(uset[eventSelected].setname)
			if setindex then
	...
	UIDropDownMenu_Initialize(setsframe, function()
		...
		if not QuickEquip.equipSets then return end--check QuickEquip.equipSets is nil value
		for k, t in pairs(QuickEquip.equipSets) do
			info.text = t.setname
			info.value = k
			info.checked = k == current
	...
	savebutton:SetScript("OnClick", function(self, button)
		...
		if setSelected and eventSelected then
			local setname = QuickEquip:GetSetNameByID(setSelected)
...
function addon:RegisterEvents()
	--reset addon event
	self:UnregisterAllEvents();
	if not QuickEquip.db.profile.enabledEE then return end
	local enabled = db.UserEvents.Enabled
